...
const Banner = () => {
  ...
  const PrevArrow = ({ onClick }) => (
    <div
      className="absolute left-4 top-1/2 transform -translate-y-1/2 z-10 cursor-pointer bg-gray-800 text-white p-3 rounded-full"
      onClick={onClick}
    >
      <IoIosArrowRoundBack size={30} />
    </div>
  ...
  const NextArrow = ({ onClick }) => (
    <div
      className="absolute right-4 top-1/2 transform -translate-y-1/2 z-10 cursor-pointer bg-gray-800 text-white p-3 rounded-full"
      onClick={onClick}
    >
      <IoIosArrowRoundForward size={30} />
    </div>
  ...
  return (
    <div className="banner mt-2 relative">
          ...
          {banners.map((data, key) => (
            <div className="banner-slider rounded-3xl" key={key}>
              <img
                src={data.banner}
                alt="Banner"
                className="rounded-3xl object-cover w-full h-64 md:h-80 lg:h-96"
                width="100%"
              />
            </div>
          ))}
